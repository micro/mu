...
func createOAuthClient(w http.ResponseWriter, r *http.Request, accountID string) {
	...
	if name == "" || len(name) > 80 || len(uris) == 0 || len(uris) > 10 {
		app.BadRequest(w, r, "Enter a name and 1–10 redirect URLs.")
		return
	}
	for _, uri := range uris {
		if !auth.RegisterableRedirect(uri) {
			app.BadRequest(w, r, "Redirect URLs must use HTTPS or HTTP on localhost.")
			return
		}
	}
	...
	http.Redirect(w, r, "/account/tokens#oauth", http.StatusSeeOther)
}
...
func oauthClients(r *http.Request, accountID string) string {
	...
	for _, c := range clients {
		...
		for _, uri := range c.RedirectURIs {
			b.WriteString(`<p>` + html.EscapeString(uri) + `</p>`)
		}
		b.WriteString(`<form method="POST" action="/account/tokens?delete_client=` + html.EscapeString(c.ClientID) + `" class="form-actions">` + app.CSRFField(auth.CSRFToken(r)) + `<input type="hidden" name="_method" value="DELETE"><button type="submit">Delete client</button></form></div>`)
	}
	b.WriteString(`<details class="disclosure"><summary>Register an OAuth client</summary><form method="POST" action="/account/tokens?create_client=1" class="form">` + app.CSRFField(auth.CSRFToken(r)) + `<label class="field-label">Name<input name="client_name" maxlength="80" required></label><label class="field-label">Redirect URLs<textarea name="redirect_uris" rows="3" required></textarea></label><p>One URL per line. HTTPS or localhost.</p><button type="submit">Register</button></form></details></section>`)
	return b.String()
}
