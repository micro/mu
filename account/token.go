...
func apiTokenForm(r *http.Request, accountID string) string {
	...
	sb.WriteString(app.Field{Name: "client", Label: "Access", Options: []app.Option{{Value: "mail", Label: "Mail", On: kind == "mail"}, {Value: "chat", Label: "Chat (XMPP)", On: kind == "xmpp"}, {Value: "api", Label: "Product API / MCP", On: agentAccess}, {Value: "services", Label: "Services API / MCP", On: serviceAccess}}}.HTML())
	sb.WriteString(`<fieldset class="scope-fields" data-token-access="api" hidden><legend>API capabilities</legend><p class="text-muted text-sm">Use /agent/api, /inbox/api and /work/api, or /agent/mcp. These credentials do not grant service tools at /mcp.</p><div class="choices"><label class="choice"><input type="checkbox" name="capability" value="api:agent"` + checked + `>Agents</label><label class="choice"><input type="checkbox" name="capability" value="api:inbox">Inbox</label><label class="choice"><input type="checkbox" name="capability" value="api:work">Background jobs</label></div><p class="text-muted text-sm">Agent access can run any of your account’s agents with their configured tools; it is not limited to one named agent. Choose Services instead to restrict a client to specific capabilities.</p><label class="choice"><input type="checkbox" name="api_write"` + checked + `>Allow actions (required to ask agents or start jobs)</label></fieldset>`)
