...
func ClientsHandler(w http.ResponseWriter, r *http.Request) {
 ...
 <h2>Mail</h2><p>Read and send Micro mail in your own mail app. This opens your Micro mailbox; it does not import another email account.</p><p><a href="/account/tokens?add=mail#create-token-form">Create mail token</a> and use it as the app’s password.</p>` + inbox.ClientSettings(acc.ID, "mail") +
		...
		`<h2>Assistant API</h2><p>Call your assistant from a script using your account’s allowance and balance. <a href="/account/tokens?add=api#create-token-form">Create an API token</a> with Agents and Allow actions selected. Set it as MICRO_TOKEN, then run:</p><pre>curl '` + base + `/api/v1/agent/ask' \
 ...
 <h2>MCP</h2><p>In a compatible app, add this remote HTTP server:</p><pre>` + base + `/mcp</pre><p>Use your API token in the header <code>Authorization: Bearer YOUR_TOKEN</code>. Call <code>agent_ask</code> with a <code>prompt</code> to talk to your assistant.</p>
