...
| `docs/` | Three embedded pages — /about, /help, /install — plus the markdown the repository keeps for itself (`ARCHITECTURE.md`, `BUSINESS_MODEL.md`, `DIRECTION.md`, `PRODUCT.md`, `PRICING.md`, `SECURITY.md`, `PRINCIPLES.md`, `USECASES.md`, `LISTING.md`), which is not served. `PRODUCT.md` is the one-pager: what Mu is, who arrives, and what Home is for. `DIRECTION.md` is where the thesis is written down and allowed to move — home server, then tools for agents, maybe agents — plus how a surface reaches a capability, and what to change next. Read it before a change that adds a door or a top-level directory. `BUSINESS_MODEL.md` evaluates those three shapes and fixes the role of x402: the inbox is the recurring product, credits meter its variable work, and x402 is the accountless machine door. `PRICING.md` is the operational companion: what things cost, who pays, and how we make money — one meter not two, when a varying price is two endpoints and when it is units, and what must never be free. Check changes against all three. The /docs route is the Docs service |
