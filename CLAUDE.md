...
| `docs/` | Two embedded pages — /about and /install — plus the markdown the repository keeps for itself (`PRICING.md`, `SECURITY.md`, `PRINCIPLES.md`, `USECASES.md`, `LISTING.md`), which is not served. There were two more, `ARCHITECTURE.md` and `PRODUCT.md`, and they are deleted: 834 lines describing what the code already says, which is a second place for the truth to live and go stale. What is registered is `/services` and `/tools`, derived from `service.Spec`; the two doors are the pages themselves; the layering is `test/layering_test.go`, which fails when it is wrong rather than being read when it is not. A doc earns its place by holding something the code cannot — a price, a policy, a threat model — which is what the five that remain do. `PRICING.md` is what things cost, who pays, and how we make money — one meter not two, when a varying price is two endpoints and when it is units, and what must never be free. Check pricing changes against it. There was a `DIRECTION.md` holding the thesis and what to change next; the thesis is the top of this file now, and a roadmap kept in the repository was a second place for the positioning to live and disagree with itself. The /docs route is the Docs service |
