...
**Work with Agents.** Make one, give it an address, hand it a job. It picks the work up while you are elsewhere and answers on the conversation you asked in. Behind it is the everyday internet — news, mail, search, weather, markets, video, storage — as tools it can call over MCP and REST, paid per request in USDC via x402, with no account in the way.

The line has moved before — *building blocks for life*, *tools for agents*, *an inbox for agents*, *a personal agent* — and the reason it kept moving is that each one described a different true thing about an artefact that is genuinely more than one thing. It was built for people first: every capability had to render a page, which is why the services are coherent, and the tools were derived from them afterwards. So it is both a catalogue and an agent, and the argument was never which one it *is* — only which one goes at the front.

"A personal agent" put the singular at the front, and the product is not singular: you make several, they have addresses, they hold work. That line also described something you talk to, and talking is the least of it.

This one is a verb, and it is a promise rather than a category — which means it can be false. It is only true while you can actually hand work over and get it back: `service/tasks` holds the work, `agent/work` runs it, and the answer returns to the thread it was asked on. Before that existed the line would have been a lie. Keep it honest: if work stops being something you can give away and collect, this sentence has to change again.

Underneath, structurally, this is a personal server with a messenger at the front — that is unchanged. Do not rewrite the first sentence when the emphasis moves; move the sentence after it.
...
- **Agents** — `agent/micro/` is what an agent is made of — a name, an instruction, a tool scope, a memory scope — and the registry of the ones this instance provides, which is one. There were eleven, routed to by keyword and by an LLM call, and a router and an orchestrator to choose between them; see the package comment for why ten of them were the same tools behind ten names. `agent/<name>/` is an agent that writes into the service of the same name: `agent/blog` composes the daily opinion by asking the registry what exists rather than naming services in code, `agent/social` surfaces breaking stories, `agent/digest` writes the daily briefing. The service stores; the agent decides what is worth storing
...
| `service/stream/` | Your stream: what has been happening here. A post published, a headline broken, a video found, mail that arrived — each one a fact the service it happened in announced on the bus (`event.Announce`), never a call into this package, because two services that name each other are one unit. It owns nothing and is the same read `service/recall` is over `internal/thread`: delete it and the news is still news. It was a console — a box you typed in and the agent answered — beside an `/agents` page and an `/inbox` that both do that better, and five of its six event types had no publisher at all |
| `service/chat/` | Live discussion rooms attached to an item |
| `internal/user/` | A person on this instance: the face they show other people (/@username), what they say they are doing, and whether they are here. Writing to somebody is `/inbox/new` at their bare address — `asim@` is the person and `asim+research@` is their agent, a line `service/mail` already draws ("untagged mail to your own address is just mail"), so there is no second channel for people and none needed. Not the account — that is `internal/auth`, which holds identity and credentials. It held a third thing, what somebody had decided about everybody else — saved, hidden, blocked, listed at /user — and that is gone with the page. Those three are the controls of a *feed*: somewhere strangers' content is put in front of you by default and you need a way to push it back. Mu has no feed, so they were not underused, they were unmotivated. Reporting survived them, at `/report`, because it is not about what the reporter sees — it tells an operator something here should not be. The word `user` is still free for the service that is missing: a directory of who exists here, people and agents alike |
| `docs/` | One page, `INSTALL.md`, served at /install. There were nine, then five, then one. The eight that went — about, product, architecture, pricing, security, principles, use cases, listing — described what the code already says, in files nothing fails when they go stale: the architecture doc kept a hand-written table of the registry with a test to stop it drifting, which is the admission that it was a copy. What is registered is `/services` and `/tools`, derived from `service.Spec`. What things cost is `quota.json`, which carries its reasoning in the notes beside each price. The layering is `test/layering_test.go`, which fails when it is wrong rather than being read when it is not. Install survives because it holds what the code cannot: ports, DNS records, an operator's decisions about a machine this repository never sees. The /docs route is the Docs service |
...
- One directory per service under `service/`, named for the service — see
  ...
  them, not a service itself. What is registered, which are account-scoped and
  which are withheld from the agent is `/services` and `/tools`, derived from
  the Specs — never a table somebody maintains. Every service has a page:
