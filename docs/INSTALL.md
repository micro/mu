...
| `TWILIO_WHATSAPP_FROM` | — | The WhatsApp sender, in E.164 (`+14155238886` is Twilio's sandbox). Point its inbound webhook at `/whatsapp/twilio` — a different path from `/whatsapp/webhook`, which belongs to the Meta client and speaks a different protocol |
