...
| `BROWSER_URL` | Optional. A Chrome DevTools endpoint for `/browser` — a Chromium container on this host, a box on the network, or a hosted browser. This is what keeps Mu a single binary: the dependency is an address rather than a program on this disk. Without it or `CHROME_PATH`, the browser service says it is not configured and serves nothing |
| `CHROME_PATH` | Optional. Path to a Chromium on this machine, for an operator who would rather run it locally than point at one. `BROWSER_URL` wins if both are set |
