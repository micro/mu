...
| `OS_MAPS_KEY` | Optional. Ordnance Survey Data Hub key, for `/tiles` — the basemap under anything spatial. Free tier at osdatahub.os.uk. Britain only. Without it the service still serves every tile this instance has already fetched, so a lapsed key degrades to the region you have already used rather than to nothing. A tile is charged once, on the first fetch this instance has ever made of it, and is free on every serve after that to anybody — tiles do not change |
