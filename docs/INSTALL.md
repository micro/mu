...
| `SANDBOX_MEMORY` · `SANDBOX_CPUS` · `SANDBOX_PIDS` | Optional, default `2g`, `1` and `512`. What one machine may have. The memory and CPU caps are what stop a build on this box from disturbing the server next to it; the process cap is against a fork bomb. Docker's own syntax, so `512m` and `0.5` are fine |
