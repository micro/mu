...
| `email` | /email | ✅ | ✅ | Email that leaves the instance, from its own sending domain, on the same Twilio credentials the texts use. A sibling of `sms` and `whatsapp` rather than half of `mail`: a mailbox is invisible outside the building, sending puts a message in front of somebody who did not ask. History asks the carrier what became of each message, so "accepted" and "delivered" are different words. `mail_email` is still how you send *as* your own address |
