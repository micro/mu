...
| **Email** | `email_send` · `email_history` · `email_sender` — send a real email to somebody outside, from a sending domain that carries nothing else, capped per day. `email_history` says what became of each one — delivered, undelivered, failed — rather than only that it was accepted. To send *as* your own address instead, so a thread holds together, see Mail below |
