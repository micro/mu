...
server — one account and token rather than hundreds.
...
| **Email** | `email_send` · `email_history` · `email_sender` — send a real email to somebody outside, from an authenticated sending domain that carries nothing else, capped per day. Replies come back to your inbox here. To send *as* your own address instead, so a thread holds together, see Mail below |
