...
| **Email** | `email_send` · `email_history` · `email_sender` · `email_verify` — send a real email to somebody outside, from a sending domain that carries nothing else, capped per day. `email_history` says what became of each one — delivered, undelivered, failed — rather than only that it was accepted. `email_verify` is the check a signup form runs — it emails a six-digit code to one of *your* users and tells you whether the code they typed back matched, under your product's name. To send *as* your own address instead, so a thread holds together, see Mail below |
