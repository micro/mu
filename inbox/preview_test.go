...
// Home shows what is in the inbox, not just a link to it.
//
...
func TestHomeShowsWhatIsInTheInbox(t *testing.T) {
	...
	if !strings.Contains(out, "session="+byMail.ID) {
	...
	// The last thing said, so the row is worth reading rather than counting.
