...
func defaultPanel(base string) string {
	...
	b.WriteString(`<div class="detail-row"><span class="text-muted">API token</span><span class="detail-value">` + app.TextLink("Create a token", "/account/tokens?add=api#create-token-form") + `. Allows calls to your account’s agents, using each agent’s configured tools.</span></div>`)
...
func connectPanel(a *Agent, base, csrf string) string {
	...
	b.WriteString(`<div class="detail-row"><span class="text-muted">Token</span><span class="detail-value"><a href="/account/tokens?add=api#create-token-form">Create a token</a></span></div>`)
