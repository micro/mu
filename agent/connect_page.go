...
func defaultPanel(base string) string {
	...
	b.WriteString(`<div class="detail-row"><span class="text-muted">API token</span><span class="detail-value">` + app.TextLink("Create a token", "/account/clients?access=agent#api-tokens") + `. Allows calls to your account’s agents, using each agent’s configured tools.</span></div>`)
	...
	b.WriteString(`<p class="conn-note">Connect a client to Agent, Work and Inbox. Call <code>agent_ask</code> with the agent name to talk to it. Create a token with Agent API access in <a href="/account/clients?access=agent#api-tokens">Clients</a>.</p>`)
...
func connectPanel(a *Agent, base, csrf string) string {
	...
	b.WriteString(`<div class="detail-row"><span class="text-muted">Token</span><span class="detail-value"><a href="/account/clients?access=agent#api-tokens">Create a token</a></span></div>`)
