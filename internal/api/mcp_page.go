...
func mcpPageHandler(w http.ResponseWriter, r *http.Request) {
	...
	b.WriteString(`if(!sel){desc.classList.add('d-none');params.innerHTML='';btn.disabled=true;return;}`)
	b.WriteString(`var t=mcpTools[sel];`)
	b.WriteString(`desc.textContent=t.description;desc.classList.remove('d-none');`)
	...
	b.WriteString(`var btn=document.getElementById('mcp-run');`)
	b.WriteString(`out.classList.remove('d-none');res.textContent='Running...';tm.textContent='';btn.disabled=true;`)
