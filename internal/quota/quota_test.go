...
// by a person in a browser, and "/account/topup" is only a destination if you
...
func TestTheTopupAddressIsOneAProgramCanFollow(t *testing.T) {
	...
	if got, want := TopupURL(), "https://micro.mu/account/topup"; got != want {
	...
	t.Setenv("PUBLIC_URL", "")
	t.Setenv("APP_URL", "")
	if got, want := TopupURL(), "/account/topup"; got != want {
