...
// balance existed only on its own page, which is one you visit because you
...
func headBalance(acc *auth.Account) string {
	...
	return fmt.Sprintf(`<a id="head-wallet" class="head-wallet%s" href="/account/billing#balance" aria-label="Account balance">`+
...
func creditsBannerFor(acc *auth.Account, path string) string {
	...
	if strings.HasPrefix(path, "/account") || strings.HasPrefix(path, "/wallet") {
		return ""
	}
...
func connectBannerFor(acc *auth.Account, path string) string {
	...
	case strings.HasPrefix(path, "/tools"),
		...
		strings.HasPrefix(path, "/account/tokens"),
