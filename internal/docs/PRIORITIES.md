...
1. **Keep live AI-news answers from leading with stale results** ([#1263](https://github.com/micro/mu/issues/1263)). The 2026-07-07 product review for #1262 found #1258 closed by PR #1261, no open codex PRs/issues, and the guest core loop still fast for weather (~2.9s), markets (~0.5s), and AI news (~0.6s). Weather now anchors correctly to Tuesday, 7 July 2026 and markets returns a readable mover list, but `Find today's AI news` still leads with May/March 2026 TechCrunch items before a 2 July item and gives no up-front no-current-results/mostly-stale caveat. This remains the highest-value Now refinement because it is the clearest first-run correctness failure in the public ask → answer loop after several shipped freshness guards; the next increment should make the deployed/native `news_search` streaming and first visible answer text obey the freshness caveat/ranking under CI-verifiable coverage.
