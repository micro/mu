...
1. **Reduce weather-backed agent answer completion latency** ([#974](https://github.com/micro/mu/issues/974)). Live guest review on 2026-07-02 found `Weather in New York today` emits early progress and a grounded, concise answer, but the one-sentence response trickles token-by-token and took about 16s to finish after the weather tool succeeded. Rank this first because the Now phase protects the core ask → answer loop as fast end to end, not only fast to first event; keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged while making simple weather-backed answers complete promptly and preserving stale/unavailable-data disclosures.

### Already shipped (do not re-queue)

- ✅ **Ground news fallback answers in source snippets.** Topic-specific unavailable-news web fallbacks now require snippet-supported stories or disclose limited evidence, closing #969 / PR #972; the remaining highest-value core-loop friction is weather-backed answer completion latency, tracked in #974.
