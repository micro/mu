...
1. **Make agent answers read like direct user-facing summaries** ([#1012](https://github.com/micro/mu/issues/1012)). Live guest review on 2026-07-02 after PR #1015 found the core ask → answer loop still fast and functional: weather answers complete in about 3s with Google Weather data, market movers complete in about 1s, and AI-news fallback completes in about 2s with web sources when `news_search` is unavailable. The remaining highest-value Now-phase friction is still presentation: the primary answer opens with implementation-style labels such as `weather` / `markets`, source metadata leads the response, and news fallback results read like a raw ranked search-results list rather than a concise answer-first summary. Preserve freshness/source metadata, provider-unavailable disclosures, links, and fast streaming while moving implementation details into secondary context and making the first sentence directly answer the user's prompt.
