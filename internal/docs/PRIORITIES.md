...
1. **Make deployed fallback answers answer-first after PR 1072** ([#1074](https://github.com/micro/mu/issues/1074)). Live guest review on 2026-07-04 after PR #1072 found the core ask → answer loop remains fast and functional, but deployed fallback answers still miss the intended answer-first bar: `Weather in New York today` completes in about 3.5s with usable Google Weather data but starts with `Dated daily forecast (provider timestamps)` instead of the current condition/temperature and today’s range; `What is moving in markets?` completes in about 0.8s and remains useful with movers/prices first; and `Find today's AI news` completes in about 1.2s using web fallback when `news_search` is unavailable but still leads with generic category/search-result pages before current story snippets. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
