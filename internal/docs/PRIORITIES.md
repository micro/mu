...
1. **Keep AI-news answers from ranking broad finance and policy as AI stories** ([#1381](https://github.com/micro/mu/issues/1381)). The 2026-07-10 product review for #1380 found #1376 already closed by PR #1379, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still says the top Now priority is seamlessness in the core ask → answer loop, not new surface area. Live first-run remains clear for guests: the landing page states `An agent for everyday`, explains the everyday-service scope, offers suggestion prompts, and keeps `/agent` usable without an account for the first questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Friday, 10 July 2026 forecast in about 1s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 1s. Sampled home/service pages continue to return 200s, `/weather` and `/chat` explain guest limits with paths back to the public agent, and `/markets` discloses source/freshness metadata. The highest-value gap is still AI-news relevance, but the live failure changed after #1379: `Find today's AI news` is fast and source/date-labeled, yet it still ranks broad same-day finance, crypto-policy, and generic developer items as normal AI-news bullets ahead of clearer AI stories because snippets mention Nvidia, AI companies, or LLMs. Refinement ranks above additions, so the next increment should require explicit AI evidence before broad same-day items can rank in AI-news answers, and should caveat sparse adjacent evidence instead of presenting it as core AI news.
