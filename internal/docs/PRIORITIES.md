...
1. **Reduce AI-news fallback answer synthesis latency** ([#987](https://github.com/micro/mu/issues/987)). Live guest review on 2026-07-02 found `Find today's AI news` emits early progress, discloses the unavailable `news_search` provider, and completes with a readable source-linked answer, but after web search finishes at about 1s the first answer token does not arrive until about 11s. Rank this first because the Now phase protects fast, correct ask → answer completion for existing agent-callable services, and the previously top-ranked market-mover latency shipped in #980 / PR #983. Keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged while making the news web-fallback synthesis stream promptly and preserving topic specificity, source links, request-date anchoring, unavailable-provider disclosure, and snippet-grounding safeguards.

### Already shipped (do not re-queue)

- ✅ **Market-mover agent answer latency reduced.** Guest `What is moving in markets?` now emits immediate market tool events and starts the final answer in under 1s in the 2026-07-02 live review, closing #980 / PR #983; the remaining highest-value core-loop friction is AI-news fallback synthesis latency, tracked in #987.
