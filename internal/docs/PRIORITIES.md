...
1. **Keep AI-news answers focused on substantive AI stories** ([#1396](https://github.com/micro/mu/issues/1396)). The 2026-07-10 product review for #1395 found #1391 closed by PR #1394, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still puts Now-phase seamlessness first: the core ask → answer loop must be fast, correct, and well-formatted before Mu adds more surface area. Live first-run remains clear for guests: the home/agent entry explains `An agent for everyday`, exposes suggestion prompts, and `/agent` says no account is needed for the first 3 questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Friday, 10 July 2026 forecast in about 1.4s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 0.8s. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s; `/chat` and `/weather` continue to explain guest/account boundaries while routing people back to the public agent. The highest-value gap remains the AI-news answer path: `Find today's AI news` is fast and source/date-labeled, and PR #1394 removed the prior UAE/Binance policy lead, but the answer still includes a generic YC hiring post and a broad SK Hynix Nasdaq-debut market story ahead of clearer Gemini-model and frontier-AI safety/governance items. Refinement ranks above additions, so the next increment should demote or caveat generic hiring, broad chip-finance/IPO/market-mover, and finance/crypto/payment-adjacent stories unless the item has concrete AI product, model, agent, governance, safety, infrastructure, or user-impact substance.
