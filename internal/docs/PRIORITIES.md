...
1. **Reduce weather-backed guest answer latency** ([#1326](https://github.com/micro/mu/issues/1326)). The 2026-07-09 product review for #1324 found #1319 closed by PR #1322, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page says Mu is “An agent for everyday”, describes the everyday services, exposes suggestion pills, and keeps the public ask box available before account creation. Guest `What is moving in markets?` remains concise and very fast (~0.5s), with source/freshness details in references. Guest `Find today's AI news` is now materially better after #1322: it stays topic-relevant, leads with a freshness caveat, and cites AI-related stories from CoinDesk, CNBC, TechCrunch, and Databricks instead of unrelated fresh feed items. Home/service pages for weather, markets, news, search, video, and chat return 200s in ~0.3-1.1s and remain reachable; signed-out `/chat` is observable as account-backed chat while the signed-out home loop remains available for public asks. The highest-value remaining Now refinement is weather latency: live guest `Weather in New York today` returned a correct, dated, source-backed Google Weather answer, but took ~17.5s end to end despite the provider succeeding. That is the top core-loop friction because it makes an otherwise correct everyday answer feel broken compared with the already-fast markets and AI-news paths.
