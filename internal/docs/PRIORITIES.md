...
1. **Keep deployed fallback answers result-first after PR 1034** ([#1038](https://github.com/micro/mu/issues/1038)). Live guest review on 2026-07-03 after PR #1034 found the core ask → answer loop is still fast and functional, but the deployed fallback answers still miss the intended result-first bar: `Weather in New York today` completes in about 2.3s with Google Weather data but starts with observation/provider-timestamp bullets and omits the visible current `30°C, Sunny` condition from the answer body; `What is moving in markets?` completes in about 1s and is mostly useful, but still needs the answer-first guard preserved without operational preamble; and `Find today's AI news` completes in about 1.4s using web fallback when `news_search` is unavailable but reads like search-result/category bullets rather than concise snippet-grounded synthesis. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
