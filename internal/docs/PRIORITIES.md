...
1. **Synthesize news web fallbacks into concise answers** ([#998](https://github.com/micro/mu/issues/998)). Live guest review on 2026-07-02 found `Find today's AI news` completing quickly and staying topic-focused when `news_search` is unavailable, but the final answer still exposes internal scaffolding (`Grounding rule`), raw result-list framing, and generic category pages instead of a polished, limited-evidence user answer. Rank this first because PR #1006 closed the higher-severity weather-unavailable defect; this is now the highest-value Now-phase ask → answer refinement with working fallback data already present. Preserve source links, topic specificity, and the human-readable unavailable-news disclosure while hiding internal grounding instructions.

### Already shipped (do not re-queue)

- ✅ **Usable weather forecasts no longer reported unavailable.** PR #1006 closed #1003: live guest `Weather in New York today` now uses Google Weather current conditions, dated forecast rows, and freshness metadata instead of ending at `Unavailable: weather_forecast`; the remaining active core-loop formatting gap is the news fallback synthesis tracked in #998.
