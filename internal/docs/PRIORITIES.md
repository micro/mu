...
1. **Make agent answers read like direct user-facing summaries** ([#1012](https://github.com/micro/mu/issues/1012)). Live guest review on 2026-07-02 found the core ask → answer loop fast and functional after PR #1010: weather answers stream in about 3s with Google Weather data, market movers stream in under 1s, and AI-news fallback returns in about 1s with web sources when `news_search` is unavailable. The remaining highest-value Now-phase friction is presentation: answers still open with tool-report scaffolding such as “Here’s what I found from the live results” and headings like `weather_forecast`, `markets`, or `Web sources` instead of direct, answer-first prose. Preserve freshness/source metadata, provider-unavailable disclosures, links, and fast streaming while hiding implementation labels from the primary answer.
