...
1. **Hide stale AI-chip finance from AI-news result cards** ([#1421](https://github.com/micro/mu/issues/1421)). The 2026-07-11 product review for #1420 found #1414 closed by PR #1419, with no open codex-labeled PRs or issues before filing this follow-up. Canon still puts Now-phase seamlessness first: the highest-value work is making the existing guest ask → answer loop correct, fast, well-formatted, and honest before adding surface area. Live first-run remains clear for guests: the landing page says `An agent for everyday`, `/agent` is directly usable, `/chat` explains saved chat requires sign-in while pointing guests to the public agent, and `/weather` explains guest weather access through the agent. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s. Guest loop checks were fast and readable: `Weather in New York today` returned a dated Saturday, 11 July 2026 forecast in about 1.2s, and `What is moving in markets?` returned priced crypto movers in about 0.5s. The remaining top friction is narrower but still visible in the core loop: `Find today's AI news` now leads with the correct no-current-results caveat for 2026-07-11 and its synthesized background bullets have improved to AI-agent, forecasting, and neurotechnology items in about 0.6s, but the attached News card still starts with stale broad SK Hynix IPO / Nasdaq finance items before the AI-substantive entries. Refinement ranks above additions, so the next increment should align the companion News card/result list with the filtered AI-news answer: keep the freshness caveat, hide or demote broad AI-chip finance unless it has concrete AI product, model, agent, infrastructure deployment, governance, safety, or user-impact substance, and preserve graceful provider-down behavior.
