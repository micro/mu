...
1. **Restore live news_search for AI-news agent prompts** ([#1154](https://github.com/micro/mu/issues/1154)). The 2026-07-05 live guest review after PR #1152 shows the core web ask → answer loop is fast and better degraded: `Weather in New York today` completed in about 3.4s with the correct Sunday, 5 July 2026 anchor; `What is moving in markets?` completed in about 0.5s with readable prices and movers; and `Find today's AI news` completed in about 1.0s with clear limited-evidence framing, full source URLs, and an explicit `news` unavailable disclosure. With #1149 now shipped by PR #1152 and no open codex PRs/issues before this review, the highest-value Now-phase gap moves upstream from fallback presentation to the provider path itself: current-news prompts still hit `news_search — unavailable`, forcing a directory/category web fallback instead of source-linked article/story answers when the news service should work end to end. Restore the news_search-backed path while preserving the fast, honest fallback when the provider is genuinely down.
