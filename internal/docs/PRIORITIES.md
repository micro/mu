...
1. **Keep live AI-news answers fresh after freshness-sorted stale results** ([#1309](https://github.com/micro/mu/issues/1309)). The 2026-07-08 product review for #1308 found #1304 closed by PR #1307, no open codex PRs, and no other open codex issues before this run. Guest `Weather in New York today` remains correct, dated to Wednesday, 8 July 2026, and reasonably fast (~4.2s with first event at ~0.8s). Guest `What is moving in markets?` remains concise and very fast (~0.7s). Guest `/chat` is clear that account-backed chat requires login while linking to the public agent, and topic summaries expose fresh metadata. The live guest `Find today's AI news` path is fast (~0.5s through `news_search`) and source-linked, but still leads a 2026-07-08 `today` request with a 14 May 2026 TechCrunch story without an up-front stale/no-current-results caveat. This stays the highest-value Now refinement because it is the only reproduced truthfulness/freshness failure in the first-run ask → answer loop after the latest stale-result sorting work: the next increment should cover the deployed native `news_search` streaming/final path so fresh dated AI-news evidence wins when available, or the answer starts with a clear mostly-stale/no-current-results disclosure before older stories.
