...
1. **Keep live AI-news today answers from leading with stale stories** ([#1212](https://github.com/micro/mu/issues/1212)). PR #1210 closed #1207 by holding native AI-news streams for the freshness guard, but the 2026-07-06 live guest review for #1211 still reproduced the core-loop defect: `Find today's AI news` reaches `news_search`, completes in about 0.8s, and keeps clean source URLs, yet both the streamed answer and final HTML still lead with May/March 2026 TechCrunch stories and give no up-front no-current-results or stale-only disclosure for a Monday, 6 July 2026 request. This remains the highest-value Now-phase PMF gap because it is the main first-run promise — ask Mu something current and get a truthful, well-formatted answer. Preserve the fast path and links; make the deployed streaming and final HTML path lead stale-only result sets with an unmistakable caveat and label older stories as background.
