...
1. **Prioritize dated story sources in AI-news fallbacks** ([#1099](https://github.com/micro/mu/issues/1099)). The 2026-07-04 live guest review shows the core web ask → answer loop is fast: weather answers in about 3.8s with the correct Saturday, 4 July 2026 anchor, markets answer in under 1s with readable movers, and AI-news fallback now date-stamps the request date after #1094/#1097. The highest-value remaining Now-phase refinement is that `Find today's AI news` still leads with generic AI-news directory pages before fresher story-like results when `news_search` is unavailable. This keeps the first-run promise focused on current, source-backed answers without adding surface area or changing public contracts.

### Already shipped (do not re-queue)

- ✅ **Lookup-only AI-news progress narration caught.** PR #1097 closed #1096 by replacing "I'll look up..." lookup-only fallback text with source-backed web results, so that completed bug is not re-queued; the remaining 2026-07-04 live friction is fallback source quality/ranking for generic AI directory pages, now tracked in #1099.
- ✅ **AI-news fallback date-stamping shipped.** PR #1094 closed #1091 and #1093 by date-stamping synthesized AI-news web fallback leads, so the previous top queue item is not re-used; the 2026-07-04 live review now sees a Saturday, 4 July 2026 (2026-07-04) lead, with the remaining generic-directory-first ranking gap tracked in #1099.

- ✅ **MCP auth session hardening shipped.** PR #1089 closed #1079, so the previous top queue item is not re-used; the 2026-07-04 product review found no open codex issues or open codex PRs after that merge, and later AI-news fallback date specificity shipped in #1094; the remaining live source-ranking friction is tracked in #1099.
