...
1. **Require concrete AI action before ranking AI-chip finance as AI news** ([#1386](https://github.com/micro/mu/issues/1386)). The 2026-07-10 product review for #1385 found #1381 and #1383 already closed by PR #1384, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still says the top Now priority is seamlessness in the core ask → answer loop, not new surface area. Live first-run remains clear for guests: the landing page states `An agent for everyday`, explains the everyday-service scope, offers suggestion prompts, and keeps `/agent` usable without an account for the first questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Friday, 10 July 2026 forecast in about 1.1s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 0.5s. Sampled home/service pages continue to return 200s, `/weather` and `/chat` explain guest limits with paths back to the public agent, and `/markets` discloses source/freshness metadata. The highest-value gap is still AI-news relevance, but the live failure changed after #1384: `Find today's AI news` is fast and source/date-labeled, yet still leads with same-day SK Hynix IPO/market stories whose AI evidence is mostly broad chip-boom or Nvidia/customer framing before clearer AI governance, agent-payment, and AI-model stories. Refinement ranks above additions, so the next increment should require concrete AI action before AI-chip finance can rank as core AI news, and should caveat sparse adjacent evidence instead of presenting it as normal AI news.
