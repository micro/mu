...
1. **Prefer article-level AI-news fallback sources** ([#1114](https://github.com/micro/mu/issues/1114)). The 2026-07-04 live guest review after PR #1112 shows the core web ask → answer loop remains fast and mostly polished: weather answers in about 3.7s with the correct Saturday, 4 July 2026 anchor, markets starts answering in under a second with readable movers, and AI-news fallback responds quickly with provider-unavailable disclosure plus source links. Since #1109 is closed, the remaining highest-value Now-phase refinement is narrower: live `Find today's AI news` can still promote a root/domain result (`The Information`) and generic category-page labels such as `AI News, Updates, Products and Reviews | Yahoo Tech` as current items when the useful evidence is only in snippets. The fallback should prefer article/story URLs and concrete story labels, or explicitly disclose limited evidence when only directory/category pages are available.
