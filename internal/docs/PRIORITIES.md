...
1. **Keep deployed AI-news answers freshness-first after PR #1290** ([#1292](https://github.com/micro/mu/issues/1292)). The 2026-07-08 product review for #1291 found #1287 closed by PR #1290, no open codex PRs, and only the lower-priority open wallet trust issue (#1188) before this run. Guest `Weather in New York today` remains correct and fast (~3.9s), and `What is moving in markets?` remains concise and fast (~1.0s). However, the live guest `Find today's AI news` path still completes quickly (~0.6s through `news_search`) while leading with May/March 2026 stories ahead of a 2 July 2026 item and without an up-front mostly-stale/no-current-results caveat. This remains the highest-value Now refinement because it is the clearest live truthfulness failure in the first-run ask → answer loop: the next increment should make the deployed/native `news_search` streaming and final synthesis paths rank fresher dated items ahead of older replayed stories and disclose stale evidence before the user sees it.
