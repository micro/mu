...
1. **Keep AI-chip finance out of stale AI-news background leads** ([#1414](https://github.com/micro/mu/issues/1414)). The 2026-07-11 product review for #1413 found #1409 closed by PR #1412, with no open codex-labeled PRs or issues before filing this follow-up. Canon still puts Now-phase seamlessness first: the highest-value work is making the existing guest ask → answer loop correct, fast, well-formatted, and honest before adding surface area. Live first-run remains clear for guests: the landing page says `An agent for everyday`, `/agent` is directly usable, `/chat` explains saved chat requires sign-in while pointing guests to the public agent, and `/weather` explains guest weather access through the agent. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s. Guest loop checks were fast and readable: `Weather in New York today` returned a dated Saturday, 11 July 2026 forecast in about 0.8s, and `What is moving in markets?` returned priced crypto movers in about 1.0s. The remaining top friction is narrower but still in the core loop: `Find today's AI news` leads with the correct no-current-results caveat for 2026-07-11 and returns in about 0.5s, but its background set still starts with broad SK Hynix Nasdaq-debut / IPO finance stories before more substantive AI-agent, forecasting, and model-governance items. Refinement ranks above additions, so the next increment should preserve the freshness caveat while demoting or omitting broad AI-chip finance background unless the story has concrete AI product, model, agent, infrastructure deployment, governance, safety, or user-impact substance.
