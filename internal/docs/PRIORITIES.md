...
1. **Keep AI-news answers topic-relevant after weather latency fix** ([#1331](https://github.com/micro/mu/issues/1331)). The 2026-07-09 product review for #1330 found #1326 closed by PR #1329, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page says Mu is “An agent for everyday”, explains news/mail/search/weather/markets/video, exposes suggestion pills, and states “No account needed for your first 3 questions.” Guest `Weather in New York today` is now correct, dated, source-backed, and fast (~1.6s), so the previous weather-latency top item is done. Guest `What is moving in markets?` remains concise and very fast (~0.7s), with readable prices and the markets card/reference context. The highest-value remaining Now refinement is AI-news topic trust: live guest `Find today's AI news` is fast (~0.5s) and source-linked, but it currently leads with unrelated fresh general-feed items (for example Army logistics/war and viral drone transport) as if they answered an AI-news request. That is more damaging than another surface addition because the core loop is only trustworthy if a same-day topical ask returns topical evidence or clearly says evidence is weak.
