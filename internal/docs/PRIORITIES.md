...
1. **Keep live AI-news answers focused after bullet polish** ([#1346](https://github.com/micro/mu/issues/1346)). The 2026-07-09 product review for #1345 found #1341 closed by PR #1344, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page states Mu is “An agent for everyday”, explains the everyday-service scope, shows suggestion pills, and tells guests “No account needed for your first 3 questions.” Guest `/chat` remains an acceptable signed-in-only degradation with account-required copy and a public-agent path. Guest `Weather in New York today` is correct, dated to Thursday, 9 July 2026, source-backed, and fast (~1.4s). Guest `What is moving in markets?` is readable, concise, and very fast (~0.5s). PR #1344 improved AI-news bullet presentation, but the live guest `Find today's AI news` path still leaks broad non-AI crypto/finance items into the canonical first-run news prompt: token-mover and crypto-policy headlines appeared before and among genuinely AI-related TechCrunch stories. The highest-value remaining Now refinement is to keep that fast path focused on AI-related stories while preserving the readable, source-linked brief format.
