...
1. **Keep live agent answers answer-first after tool calls** ([#1021](https://github.com/micro/mu/issues/1021)). Live guest review on 2026-07-03 after PR #1019 found the core ask → answer loop remains fast and functional: weather completes in about 3s with Google Weather data, market movers complete in about 0.5s, and AI-news fallback completes in about 1.3s with web sources when `news_search` is unavailable. The remaining highest-value Now-phase friction is still answer presentation: weather and markets final answers lead with request-date/source metadata before the answer, and the AI-news fallback still reads like a numbered search-results list instead of a concise answer-first synthesis. Preserve freshness/source metadata, provider-unavailable disclosures, links, and fast streaming while moving implementation details into secondary context and making the first sentence directly answer the user's prompt.
