...
1. **Fix live answer-first regression after fallback polish** ([#1026](https://github.com/micro/mu/issues/1026)). Live guest review on 2026-07-03 after PR #1024 found the core ask → answer loop remains fast and functional, but the deployed answers still miss the intended answer-first bar: `Weather in New York today` completes in about 4.1s with Google Weather data but starts with request-date/provider timestamp bullets and omits the current 32°C clear condition from the visible answer; `What is moving in markets?` completes in about 0.8s but still leads with the request date before movers; and `Find today's AI news` completes in about 1.0s using web fallback when `news_search` is unavailable but still reads like category/search-result bullets instead of a concise snippet-grounded synthesis. Preserve the shipped latency, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
