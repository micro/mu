...
1. **Keep same-day AI news focused on explicit AI stories** ([#1376](https://github.com/micro/mu/issues/1376)). The 2026-07-10 product review for #1375 found #1371 already closed by PR #1374, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still says the top Now priority is seamlessness in the core ask → answer loop, not new surface area. Live first-run remains clear for guests: the landing page states `An agent for everyday`, explains the everyday-service scope, offers suggestion prompts, and keeps `/agent` usable without an account for the first questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Friday, 10 July 2026 forecast in about 1.7s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 0.3s. Sampled home/service pages continue to return 200s. The highest-value gap is still AI-news relevance, but the live failure changed after #1374: `Find today's AI news` no longer led with crypto market background, yet it promoted unrelated/weakly adjacent same-day items ahead of explicit AI stories (for example an IR TV-volume Windows tool, a generic maintainable-code essay, and a UK politics story before OpenAI and AI-agent startup items). Refinement ranks above additions, so the next increment should require explicit AI evidence before same-day items can rank in AI-news answers while preserving speed, source/date labels, links, and graceful provider degradation.
