...
1. **Disclose stale-only news results before today story lists** ([#1187](https://github.com/micro/mu/issues/1187)). PR #1184 closed #1172 and #1183 by widening the current-news candidate pool, so those completed issues should not stay queued. The 2026-07-06 live guest review for #1186 still shows the highest-value Now-phase friction in the core ask → answer loop: `Find today's AI news` reaches `news_search`, completes in under 1s, and keeps clean source URLs, but the rendered answer still leads with March–May 2026 TechCrunch/CoinDesk stories for a "today" prompt on Monday, 6 July 2026 with no up-front stale-only caveat. Keep the fast provider path and clean links; make same-day/recent evidence win when available, and when the result set is stale-only, put an unmissable no-current-results disclosure before the story list.
