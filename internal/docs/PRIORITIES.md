...
1. **Keep same-day AI news from promoting crypto market background** ([#1371](https://github.com/micro/mu/issues/1371)). The 2026-07-10 product review for #1370 found #1364 closed by PR #1367, and no open codex-labeled issues or PRs before filing this follow-up. The canon still points to refinement over new surface area: protect the guest ask → answer loop first. Live guest first-run remains clear on the landing page (`An agent for everyday`, everyday-service scope, suggestion prompts, and `No account needed for your first 3 questions`). Guest `/agent` answers work without login, while `/chat` is observable as a signed-in chat surface and guest entry point. Core non-news prompts remain fast and readable: `Weather in New York today` returns a dated Friday, 10 July 2026 forecast in about 2.0s, and `What is moving in markets?` returns top movers in about 0.3s with prices and 24h changes. Sampled home/service pages (`/home`, `/news`, `/weather`, `/markets`, `/video`, `/search`, `/agent`, `/chat`, `/status`) return 200s in roughly 0.4–1.0s. The highest-value Now gap remains AI-news relevance in the core loop: live `Find today's AI news` is fast, has freshness caveats and clean labels, and includes concrete AI items, but it still promotes an unrelated same-day crypto market-mover story as `Background` ahead of/on par with explicit OpenAI, Fed AI, and AI-agent stories. Refinement ranks above additions, so the next increment should keep broad crypto/market background out of same-day AI-news bullets unless explicit AI evidence exists, preserving speed, links, and freshness handling.
