...
1. **Polish live AI-news caveat answer formatting** ([#1354](https://github.com/micro/mu/issues/1354)). The 2026-07-10 product review for #1353 found #1346 closed by PR #1349, the follow-up token-portfolio filter shipped in PR #1352 / #1351, and no other open codex-labeled issues or PRs before this run. Guest first-run framing remains clear on the landing page: “An agent for everyday,” everyday-service scope, suggestion pills, and “No account needed for your first 3 questions.” Guest `/chat` and `/agent` are reachable, and `/chat` returns the same shared agent UI rather than a raw auth failure. Live guest core prompts are fast: `Weather in New York today` returns a correct Friday, 10 July 2026 answer in about 1.6s; `What is moving in markets?` returns readable movers in about 0.9s; `Find today's AI news` returns in about 1.2s with an appropriate no-same-day freshness caveat and focused AI-related 9 July background items. The remaining highest-value Now refinement is formatting: the AI-news main answer still reads like provider rows (`category:`, `posted:`, full `source:` URLs, and truncated snippets) instead of clean story bullets with concise source/date labels while keeping raw evidence in references.
