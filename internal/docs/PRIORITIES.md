...
1. **Reduce market-mover agent answer latency** ([#980](https://github.com/micro/mu/issues/980)). Live guest review on 2026-07-02 found `What is moving in markets?` emits a flow id quickly, but waits about 5s before the first market tool event and about 14s before the final answer starts; the answer is readable and source-linked, but the end-to-end wait is now the highest-value core-loop friction after weather latency shipped in #974 / PR #978. Rank this first because the Now phase protects fast, correct ask → answer completion for existing agent-callable services; keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged while reducing avoidable market-answer stalls and preserving concise formatting, source links, provider disclosures, and dollar-price rendering.

### Already shipped (do not re-queue)

- ✅ **Weather-backed agent answer completion latency reduced.** Guest `Weather in New York today` now emits early progress and completes in about 3.5s when the weather provider reports unavailable, closing #974 / PR #978; the remaining highest-value core-loop friction is market-mover answer latency, tracked in #980.
