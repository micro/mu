...
1. **Anchor deployed weather fallback answers to the request date** ([#1081](https://github.com/micro/mu/issues/1081)). Live guest review on 2026-07-04 after PR #1077 found the core ask → answer loop remains fast, but the highest-value correctness gap is now weather date anchoring: `Weather in New York today` completes in about 3.1s with Google Weather current conditions and the correct request-date context in references, yet the visible answer begins `Today: Friday, 3 July 2026` even though the request date is Saturday, 4 July 2026. Preserve the shipped latency, tool progress, source/freshness metadata, and provider-unavailable transparency while ensuring relative-day weather answers lead with the request-date current condition/forecast, not the first provider row.

2. **Require sessions before invoking MCP auth handlers** ([#1079](https://github.com/micro/mu/issues/1079)). This is not a new product surface, but an active codex-labelled hardening item that protects authenticated service operations from running with an empty account binding. Keep it below the live core-loop correctness regression because the current phase ranks first-run ask → answer seamlessness highest, but do not drop it while it remains open.
