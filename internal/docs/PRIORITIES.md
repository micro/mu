...
1. **Keep native AI-news answers freshness-first after PR 1312** ([#1314](https://github.com/micro/mu/issues/1314)). The 2026-07-09 product review for #1313 found #1309 closed by PR #1312, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page says what Mu does, exposes useful suggestion pills, and states no account is needed for the first 3 questions. Guest `Weather in New York today` is correct, dated to Thursday, 9 July 2026, source-backed by Google Weather, and completes in ~3.4s with first SSE at ~1.0s. Guest `What is moving in markets?` remains concise and very fast (~0.6s). Guest `/chat` is observable as account-backed chat with the public agent path still available from the signed-out home loop. Home/service pages for weather, markets, news, search, and video return 200s in ~0.5-1.1s and remain reachable. The live guest `Find today's AI news` path is fast (~0.5s through `news_search`) and source-linked, but still leads a 2026-07-09 `today` request with 14 May, 11 May, and 12 March 2026 stories before a 2 July 2026 item, with no up-front stale/no-current-results caveat. This remains the highest-value Now refinement because it is the only reproduced truthfulness/freshness failure in the first-run ask → answer loop after the latest native freshness-caveat work: the next increment should make the native `news_search` streamed and final answer freshness-first, or lead with an explicit mostly-stale/no-current-results disclosure before older stories.
