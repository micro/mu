...
1. **Surface stale-only caveat in streamed AI-news answers** ([#1207](https://github.com/micro/mu/issues/1207)). PR #1205 closed #1202 by guarding another stale-news native streaming path, but the 2026-07-06 live guest review for #1206 still reproduced the core-loop defect: `Find today's AI news` reaches `news_search`, completes in about 0.7s, and keeps clean source URLs, yet the first streamed answer still leads with May/March 2026 TechCrunch/CoinDesk stories before a 2 July 2026 result and gives no up-front no-current-results or stale-only disclosure for a Monday, 6 July 2026 request. This remains the highest-value Now-phase PMF gap because it is the main first-run promise — ask Mu something current and get a truthful, well-formatted answer. Preserve the fast path and links; make the deployed streaming and final HTML path lead stale-only result sets with an unmistakable caveat and label older stories as background.
