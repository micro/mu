...
1. **Keep AI-news limited-evidence fallback links readable** ([#1124](https://github.com/micro/mu/issues/1124)). The 2026-07-04 live guest review after PR #1122 shows the core web ask → answer loop remains fast and mostly polished: weather answers in about 3.5s with the correct Saturday, 4 July 2026 anchor, markets answers in under 1s with readable movers, and AI-news fallback responds in about 1.5s with provider-unavailable disclosure and explicit limited-evidence labels for root/category sources. Since #1119 is closed, the remaining highest-value Now-phase refinement is the still-live fallback presentation gap: `Find today's AI news` can render a truncated or malformed source URL such as `https://www.…` for a limited-evidence item. The fallback should keep limited-evidence disclosure, but render full usable URLs or concrete article URLs when available.
