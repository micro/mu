...
1. **Filter stale and off-topic AI-news leftovers** ([#1404](https://github.com/micro/mu/issues/1404)). The 2026-07-11 product review for #1403 found #1396 closed by PRs #1400 and #1402, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still puts Now-phase seamlessness first: the core ask → answer loop must be fast, correct, and well-formatted before Mu adds more surface area. Live first-run remains clear for guests: the home/agent entry says `An agent for everyday`, offers suggestion prompts, and `/agent` says no account is needed for the first 3 questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Saturday, 11 July 2026 forecast in about 1.2s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 0.5s. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s; `/chat` and `/weather` continue to explain guest/account boundaries while routing people back to the public agent. The highest-value remaining gap is still in the AI-news answer path, but narrower after the shipped finance/hiring filters: `Find today's AI news` is very fast and source/date-labeled, yet when only 3 of 8 dated results are same-day it still lists stale/background items and an off-topic `FreeCAD in the Browser` result alongside substantive AI stories. Refinement ranks above additions, so the next increment should keep the freshness caveat, demote stale/background context, and filter generic software/tool posts unless the item has concrete AI product, model, agent, governance, safety, infrastructure, or user-impact substance.
...
- ✅ **Substantive AI-news and hiring filters shipped but stale/off-topic leftovers remain.** PR #1400 closed #1396 by tightening topic filtering, and PR #1402 added indexed hiring-filter coverage. The 2026-07-11 live review for #1403 confirmed `Find today's AI news` is fast and now caveats mostly stale results, but it still lists stale/background items and an off-topic `FreeCAD in the Browser` result beside substantive AI stories, now tracked in #1404.
