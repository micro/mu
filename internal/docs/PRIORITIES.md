...
1. **Make live fallback answers directly answer first after PR 1046** ([#1048](https://github.com/micro/mu/issues/1048)). Live guest review on 2026-07-03 after PR #1046 found the core ask → answer loop is still fast and functional, but the deployed fallback answers still miss the intended result-first bar: `Weather in New York today` completes in about 4.2s with weather data but starts with observation/provider-timestamp bullets instead of a plain current-conditions answer; `What is moving in markets?` completes in about 0.8s and remains mostly useful with movers/prices first; and `Find today's AI news` completes in about 1.5s using web fallback when `news_search` is unavailable but still reads like category/search-result pages rather than concise snippet-grounded current news. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
