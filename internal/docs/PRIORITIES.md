...
1. **Keep AI-news fallback from promoting limited-evidence category snippets as lead stories** ([#1149](https://github.com/micro/mu/issues/1149)). The 2026-07-05 live guest review after PR #1147 shows the web ask → answer loop remains fast and mostly useful: weather answers in about 3.4s with the correct Sunday, 5 July 2026 anchor; markets answers in about 0.6s with readable movers and prices; and AI-news fallback responds in about 1.2s with provider-unavailable disclosure and full source URLs. The remaining highest-value Now-phase friction is still in the same core fallback path: `Find today's AI news` promoted a limited-evidence category URL (`tech.yahoo.com/ai/`) as the lead story, duplicated the Meta AI-glasses snippet around an em dash, and only then disclosed that news was unavailable. Since #1144 shipped in PR #1147 and there are no open codex PRs/issues, the next refinement should keep category/root/topic evidence in explicit limited-evidence framing unless an article-like source supports a concrete story, while preserving fast graceful degradation.
