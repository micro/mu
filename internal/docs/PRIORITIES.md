...
1. **Keep mostly stale AI-news answers caveated after merge** ([#1226](https://github.com/micro/mu/issues/1226)). PR #1224 closed #1221, but the live 2026-07-07 guest review still saw `Find today's AI news` complete quickly through `news_search` while the first streamed answer led with March-May 2026 stories before a 2 July item and no up-front mostly-stale/no-current-results caveat. This remains the highest-value Now-phase refinement because it is the first-run ask → answer loop: preserve the fast source-linked path, but make the first streamed token and final HTML disclose when current-news evidence is mostly stale, with regression coverage for the deployed failure mode.
