...
1. **Keep AI-news leads and cards topic-relevant** ([#1431](https://github.com/micro/mu/issues/1431)). The 2026-07-11 product review for #1430 found #1426 closed, with no open codex-labeled PRs or issues before filing this follow-up. Canon still puts Now-phase seamlessness first: the highest-value work remains the existing guest ask → answer loop being correct, fast, well-formatted, and honest before any new surface area. Live first-run remains clear for guests: the landing page says `An agent for everyday`, `/agent` is directly usable, `/chat` explains saved chat requires sign-in while pointing guests to the public agent, and `/weather` explains guest weather access through the agent. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s. Guest loop checks were fast and readable: `Weather in New York today` returned a dated Saturday, 11 July 2026 forecast with `weather_forecast` in about 1.7s, and `What is moving in markets?` returned priced crypto movers with `markets` in about 0.5s. The remaining top friction is narrower but still core-loop correctness: `Find today's AI news` completed through `news_search` in about 0.4s and correctly warned that only 1 of 6 dated results were from 2026-07-11, but the synthesized answer promoted a same-day Amazon layoff/job-market story and an older HotSpot JIT/compiler item ahead of substantive AI-agent context, while the attached News card also surfaced an SK Hynix IPO finance item. Refinement ranks above additions, so the next increment should keep the freshness caveat, filter/rank AI-news leads for concrete AI product/model/agent/infrastructure/governance/safety/user-impact substance, align companion cards with the synthesized AI-focused answer, and preserve graceful provider-down behavior.
