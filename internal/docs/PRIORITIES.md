...
1. **Make deployed fallback answers answer-first after PR 1056** ([#1058](https://github.com/micro/mu/issues/1058)). Live guest review on 2026-07-03 after PR #1056 found the core ask → answer loop is still fast and functional, but the deployed fallback answers still miss the intended answer-first bar: `Weather in New York today` completes in about 4.0s with weather data but starts with observation/provider-timestamp bullets instead of a plain current-conditions answer; `What is moving in markets?` completes in about 0.7s and remains useful with movers/prices first; and `Find today's AI news` completes in about 1.4s using web fallback when `news_search` is unavailable but still leads with generic category/search-result pages rather than concise snippet-grounded current news. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
