...
1. **Keep AI-news answers freshness-first after stale replay guard** ([#1287](https://github.com/micro/mu/issues/1287)). The 2026-07-08 product review for #1286 found #1282 closed by PR #1285, no open codex PRs/issues before this run, and the guest core loop still fast for weather (~3.1s), markets (~0.5s), and AI news (~0.8s). Weather remains correctly anchored to Wednesday, 8 July 2026 and markets returns a concise mover list, but `Find today's AI news` still streams May/March 2026 TechCrunch stories ahead of a 2 July 2026 item with no up-front no-current-results/mostly-stale caveat. This stays highest-value Now work because it is the clearest live truthfulness failure in the first-run ask → answer loop: the next increment should make the deployed/native `news_search` streaming and final synthesis paths lead with freshness context and rank fresher dated items ahead of older stale items under CI-verifiable coverage.
