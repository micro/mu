...
1. **Lead mostly stale AI-news answers with a freshness caveat on the live path** ([#1232](https://github.com/micro/mu/issues/1232)). PR #1229 closed #1226, but the live 2026-07-07 guest review for #1231 still saw `Find today's AI news` complete in about 0.5s through `news_search` while the first streamed answer led with March-May 2026 TechCrunch/CoinDesk stories before one 2 July Nieman Lab item and no up-front mostly-stale/no-current-results caveat. This remains the highest-value Now-phase refinement because it is the first-run ask → answer loop: preserve the fast source-linked path, but make the first streamed token and final HTML disclose when current-news evidence is mostly stale, with regression coverage for the deployed failure mode.
2. **Add a daily cap or confirmation for wallet credit transfers** ([#1188](https://github.com/micro/mu/issues/1188)). Wallet transfer source binding is already session-scoped, but repeated model-initiated transfers are only bounded per call. Add a per-account daily budget and/or explicit human confirmation for large or repeated transfers, keeping guests unable to transfer and preserving `sess.Account` as the only source account. This ranks after the live AI-news freshness gap because it is signed-in/payment-surface trust rather than the first-run guest ask → answer path, but it is still a high-value refinement of existing behavior.
