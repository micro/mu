...
1. **Prefer article-level sources in AI-news unavailable fallbacks** ([#1130](https://github.com/micro/mu/issues/1130)). The 2026-07-05 live guest review after PR #1128 shows the core web ask → answer loop remains fast and mostly polished: weather answers in about 4s with the correct Sunday, 5 July 2026 anchor, markets answers in under 1s with readable movers and prices, and AI-news fallback responds in about 1s with provider-unavailable disclosure, explicit limited-evidence labels, and now-readable full URLs. Since #1124 is closed, the remaining highest-value Now-phase refinement is that `Find today's AI news` can still present category/root pages such as Yahoo AI, Guardian artificial-intelligence, or Bloomberg AI as source-backed latest items when `news_search` is unavailable. The fallback should prefer concrete article URLs when available, or keep the limited-evidence framing without overclaiming category pages as verified current stories.
