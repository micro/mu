...
1. **Keep deployed AI-news answers from leading with stale dotted-tool results** ([#1270](https://github.com/micro/mu/issues/1270)). The 2026-07-08 product review for #1269 found #1263 closed by PR #1268, no open codex PRs/issues, and the guest core loop still fast for weather (~3.3s), markets (~0.9s), and AI news (~1.3s). Weather now anchors correctly to Wednesday, 8 July 2026 and markets returns a readable mover list, but `Find today's AI news` still leads with May/March 2026 TechCrunch items before a 2 July item and gives no up-front no-current-results/mostly-stale caveat. This remains the highest-value Now refinement because it is the clearest first-run correctness failure in the public ask → answer loop after several shipped freshness guards; the next increment should make the deployed/native `news_search` and dotted-tool streaming paths ensure first visible answer text obeys the freshness caveat/ranking under CI-verifiable coverage.
