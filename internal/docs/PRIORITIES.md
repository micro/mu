...
1. **Keep AI-news searches topic-relevant after PR 1317** ([#1319](https://github.com/micro/mu/issues/1319)). The 2026-07-09 product review for #1318 found #1314 closed by PR #1317, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page says Mu is “An agent for everyday”, lists the everyday services it can handle, exposes suggestion pills, and keeps the public ask box available before account creation. Guest `Weather in New York today` is correct, dated to Thursday, 9 July 2026, source-backed by Google Weather, and completes in ~3.0s. Guest `What is moving in markets?` remains concise and very fast (~0.5s). Guest `/chat` is observable as account-backed chat while the signed-out home loop remains available for public agent asks. Home/service pages for weather, markets, news, search, video, and chat return 200s in ~0.3-1.1s and remain reachable. The live guest `Find today's AI news` path is fast (~0.8s through `news_search`) and now freshness-first, but it over-corrects toward freshest feed items regardless of topic: the answer leads with Quran/hadith/name-of-God reminder entries plus finance headlines before one AI-adjacent developer article. This is now the highest-value Now refinement because the reproduced core-loop failure moved from freshness to truthfulness/topic relevance: a user asking for AI news gets mostly unrelated fresh items. The next increment should keep fresh AI-relevant stories ahead of unrelated fresh feed entries, or open with an explicit no-current/limited-evidence caveat when the available AI evidence is weak.
