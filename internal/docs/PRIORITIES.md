...
1. **Keep generic finance policy out of AI-news leads** ([#1391](https://github.com/micro/mu/issues/1391)). The 2026-07-10 product review for #1390 found #1386 closed by PR #1389, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still puts Now-phase seamlessness first: protect the core ask → answer loop before adding surface area. Live first-run remains clear for guests: the landing page states `An agent for everyday`, explains the everyday-service scope, exposes suggestion prompts, and `/agent` remains usable without an account for initial questions. Guest core-loop checks remain fast and readable: `Weather in New York today` returned a dated Friday, 10 July 2026 forecast in about 1.3s, and `What is moving in markets?` returned top crypto movers with prices and 24h changes in about 0.3s. Sampled home/service pages returned 200s, `/weather` and `/chat` continue to explain guest limits with paths back to the public agent, and `/markets` continues to disclose source/freshness metadata. The highest-value gap remains AI-news relevance, now shifted from AI-chip IPO leakage to broader generic finance/policy leakage: `Find today's AI news` is fast and source/date-labeled, but still leads with a UAE export-control / Binance-stablecoin finance-policy story before clearer AI-agent payments and AI-model evaluation stories, then follows with SK Hynix IPO/market items whose AI evidence is still mostly broad chip-boom or Nvidia/customer framing. Refinement ranks above additions, so the next increment should demote or caveat generic finance, crypto-policy, export-control, IPO, and market stories unless they have concrete AI action, AI product/model behavior, AI agent/tool impact, or explicit AI governance substance.
