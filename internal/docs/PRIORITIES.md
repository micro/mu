...
1. **Ensure AI-news freshness caveats survive native stream deployment** ([#1275](https://github.com/micro/mu/issues/1275)). The 2026-07-08 product review for #1274 found #1270 closed by PR #1273, no open codex PRs/issues before this run, and the guest core loop still fast for weather (~2.9s), markets (~0.5s), and AI news (~0.5s). Weather now anchors correctly to Wednesday, 8 July 2026 and markets returns a readable mover list, but `Find today's AI news` still leads with May/March 2026 TechCrunch items before a 2 July item and gives no up-front no-current-results/mostly-stale caveat. This remains the highest-value Now refinement because it is still the clearest first-run truthfulness failure in the public ask → answer loop after the latest native-stream hold; the next increment should make the deployed/native `news_search` streaming and final synthesis paths ensure first visible answer text obeys the freshness caveat/ranking under CI-verifiable coverage.
