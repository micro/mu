...
1. **Keep stale AI-news items behind freshness caveats after guarded replay** ([#1282](https://github.com/micro/mu/issues/1282)). The 2026-07-08 product review for #1281 found #1275 closed by PR #1278, no open codex PRs/issues before this run, and the guest core loop still fast for weather (~3.0s), markets (~0.5s), and AI news (~0.6s). Weather now anchors correctly to Wednesday, 8 July 2026 and markets returns a readable mover list, but `Find today's AI news` still streams May/March 2026 TechCrunch stories before a 2 July 2026 item and gives no up-front no-current-results/mostly-stale caveat. This remains the highest-value Now refinement because it is still the clearest first-run truthfulness failure in the public ask → answer loop after the guarded native replay work; the next increment should make the deployed/native `news_search` streaming and final synthesis paths ensure first visible answer text obeys the freshness caveat/ranking under CI-verifiable coverage.
