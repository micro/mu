...
1. **Filter generic AI-news directory pages from live fallbacks** ([#1104](https://github.com/micro/mu/issues/1104)). The 2026-07-04 live guest review shows the core web ask → answer loop is fast and mostly polished: weather answers in about 2.9s with the correct Saturday, 4 July 2026 anchor, markets answer in under 1s with readable movers, and AI-news fallback responds in about 1.2s with provider-unavailable disclosure plus source links. After #1099 / PR #1102, the remaining highest-value Now-phase refinement is that live `Find today's AI news` can still lead with generic AI category/directory pages (`artificialintelligence-news.com`, Yahoo AI, TechCrunch AI) alongside one story-like item when `news_search` is unavailable. Demoting or filtering generic directories keeps the first-run promise focused on current, source-backed answers without adding surface area or changing public contracts.
