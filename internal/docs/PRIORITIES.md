...
1. **Ensure deployed AI-news answers don't stream stale items first after PR #1297** ([#1299](https://github.com/micro/mu/issues/1299)). The 2026-07-08 product review for #1298 found #1292 closed by PR #1297, no open codex PRs, and no other open codex issues before this run. Guest `Weather in New York today` remains correct and fast (~3.2s), and `What is moving in markets?` remains concise and fast (~0.5s). However, the live guest `Find today's AI news` path still completes quickly (~0.8s through `news_search`) while leading with May/March 2026 stories ahead of a 2 July 2026 item and without an up-front mostly-stale/no-current-results caveat. This remains the highest-value Now refinement because it is the clearest live truthfulness failure in the first-run ask → answer loop after the latest freshness patch: the next increment should cover the deployed-style streaming/final synthesis path so fresh dated items are not buried behind stale replayed stories, or a stale-results caveat appears before older evidence.
