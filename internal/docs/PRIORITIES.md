...
1. **Keep news_search-backed AI-news answers fast and source-clean** ([#1162](https://github.com/micro/mu/issues/1162)). The 2026-07-05 live guest review after PR #1160 shows the highest-value Now-phase gap has moved from provider availability to seamless answer quality on the restored provider path. `Weather in New York today` completed in about 2.6s with the correct Sunday, 5 July 2026 anchor and dated forecast rows, and `What is moving in markets?` completed in about 0.6s with readable prices. `Find today's AI news` now correctly reaches `news_search` instead of the old unavailable fallback, but the run took about 21s to finish and the rendered References panel exposed a large escaped JSON-like provider payload with truncated URL data; one surfaced link also pointed at an image asset rather than the article URL. Keep the restored news_search path, but make it fast, bounded, and source-clean so the core ask → answer loop feels seamless rather than like a raw tool report.
