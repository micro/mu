...
1. **Keep same-day AI-news answers topic-focused** ([#1359](https://github.com/micro/mu/issues/1359)). The 2026-07-10 product review for #1358 found #1354 closed by PR #1357, no open codex-labeled issues or PRs before filing this item, and the clean story-label formatting improvement live. Guest first-run framing remains clear on the landing page: “An agent for everyday,” everyday-service scope, suggestion prompt, and login is optional for initial exploration. Guest `/chat` and `/agent` remain reachable without a raw auth failure, and core non-news prompts are fast and readable: `Weather in New York today` returns a dated Friday, 10 July 2026 forecast in about 1s, while `What is moving in markets?` returns top movers in about 1s with prices and 24h moves. The highest-value Now gap is still inside the ask → answer loop: live `Find today's AI news` is fast and now cleaner, but it again leads with broad SK Hynix finance, Bitcoin/crypto, and unrelated consumer-internet headlines alongside genuine AI stories. Refinement ranks above new surface area, so the next increment should keep same-day AI-news result selection topic-focused while preserving the shipped clean bullet formatting and graceful caveats.
