...
1. **Keep deployed fallback answers result-first after PR 1041** ([#1043](https://github.com/micro/mu/issues/1043)). Live guest review on 2026-07-03 after PR #1041 found the core ask → answer loop is still fast and functional, but the deployed fallback answers still miss the intended result-first bar: `Weather in New York today` completes in about 2.5s with Google Weather data but starts with observation/provider-timestamp bullets instead of a current-conditions answer; `What is moving in markets?` completes in under 1s and is mostly useful, with movers/prices first; and `Find today's AI news` completes in about 1.8s using web fallback when `news_search` is unavailable but still reads like search-result/category-page bullets rather than concise snippet-grounded synthesis. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
