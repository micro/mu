...
1. **Use successful weather tool results in agent answers** ([#997](https://github.com/micro/mu/issues/997)). Live guest review on 2026-07-02 found PR #995 closed #992 and the weather tool now returns usable Google Weather forecast rows for `Weather in New York today`, but the final answer still says `Unavailable: weather_forecast` even while the References card contains current conditions, dated forecast rows, and freshness metadata. Rank this first because weather is a core everyday service in the Now phase and this is the highest-friction ask → answer failure: Mu has the right data, but gives the user the wrong answer. Keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged, preserve fast true-provider-failure degradation, and render a concise location/date-specific answer with source/freshness when the tool payload is usable.
2. **Synthesize news web fallbacks into concise answers** ([#998](https://github.com/micro/mu/issues/998)). Guest `Find today's AI news` now completes quickly and remains topic-focused when `news_search` is unavailable, but the final answer still exposes internal scaffolding (`Grounding rule`, raw result-list framing, and generic category pages) instead of a polished, limited-evidence user answer. Rank this after weather because it is also in the core loop and already has working fallback data; the remaining gap is refinement of formatting/truthfulness rather than new surface area. Preserve source links, topic specificity, and the human-readable unavailable-news disclosure while hiding internal grounding instructions.
