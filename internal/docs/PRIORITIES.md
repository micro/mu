...
1. **Restore deployed stale-only disclosure in AI-news answers** ([#1202](https://github.com/micro/mu/issues/1202)). PR #1200 closed #1197, but the 2026-07-06 live guest review for #1201 still reproduced the same deployed core-loop defect: `Find today's AI news` reaches `news_search`, completes in about 0.5s, and keeps clean source URLs, yet the first streamed answer still leads with March-May 2026 TechCrunch/CoinDesk stories with no up-front no-current-results or stale-only disclosure for a Monday, 6 July 2026 request. This remains the highest-value Now-phase PMF gap because it is the main first-run promise — ask Mu something current and get a truthful, well-formatted answer. Preserve the fast path and links; make the deployed synthesis/rendering path lead stale-only result sets with an unmistakable caveat and label older stories as background.
