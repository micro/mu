...
1. **Require explicit AI relevance for AI-news bullets** ([#1364](https://github.com/micro/mu/issues/1364)). The 2026-07-10 product review for #1363 found #1359 / #1361 closed by PR #1362, with no open codex-labeled issues or PRs before filing this item. Guest first-run framing remains clear on the landing page: “An agent for everyday,” everyday-service scope, suggestion prompts, and the explicit “No account needed for your first 3 questions” hint. Guest `/agent` works without login and guest `/chat` degrades gracefully with a saved-chat login notice plus a public-agent path. Core non-news prompts remain fast and readable: `Weather in New York today` returns a dated Friday, 10 July 2026 forecast in about 1.5s, and `What is moving in markets?` returns top movers in under 1s with prices and 24h changes. Home/service pages sampled (`/home`, `/news`, `/weather`, `/markets`, `/video`, `/search`, `/status`) respond with 200s in under about 0.7s. The highest-value Now gap is still inside the core ask → answer loop: live `Find today's AI news` is fast and cleaner, but after the merged broad-finance filter it still presents weakly related same-day background items such as SK Hynix IPO finance and fake portable-air-conditioner internet ads before/on par with concrete OpenAI and AI-agent stories. Refinement ranks above new surface area, so the next increment should require explicit AI relevance for each AI-news bullet while preserving freshness caveats, clean story labels, links, and the fast path.
