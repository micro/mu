...
1. **Use story titles for AI-news web fallback leads** ([#1109](https://github.com/micro/mu/issues/1109)). The 2026-07-04 live guest review shows the core web ask → answer loop is fast and mostly polished: weather answers in about 3.4s with the correct Saturday, 4 July 2026 anchor, markets answer in under 1s with readable movers, and AI-news fallback responds in about 1.0–1.4s with provider-unavailable disclosure plus source links. After #1104 / PR #1107 filtered generic AI directory pages, the remaining highest-value Now-phase refinement is that live `Find today's AI news` can still promote generic category-page titles such as `AI News, Updates, Products and Reviews | Yahoo Tech` as a current item when the useful story is only in the snippet. The fallback should lead with concrete snippet-supported story descriptions, or disclose limited evidence when only directory/category pages are available.
