...
1. **Make deployed fallback answers answer-first after PR 1066** ([#1068](https://github.com/micro/mu/issues/1068)). Live guest review on 2026-07-03 after PR #1066 found the core ask → answer loop remains fast and functional, but deployed fallback answers still miss the intended answer-first bar: `Weather in New York today` completes in about 3.9s with usable Google Weather data but starts with observation/provider timestamp and dated forecast metadata instead of the current condition/temperature and today’s range; `What is moving in markets?` completes in about 0.6s and remains useful with movers/prices first; and `Find today's AI news` completes in about 1.2s using web fallback when `news_search` is unavailable but still leads with generic category/search-result pages before current story snippets. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
