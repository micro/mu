...
1. **Avoid category-page leads in AI-news unavailable fallbacks** ([#1137](https://github.com/micro/mu/issues/1137)). The 2026-07-05 live guest review after PR #1133 shows the core web ask → answer loop remains fast and mostly polished: weather answers in about 3s with the correct Sunday, 5 July 2026 anchor; markets answers in under 1s with readable movers and prices; and AI-news fallback responds in about 1.5s with provider-unavailable disclosure, explicit limited-evidence labels, and readable full URLs. However, `Find today's AI news` still leads with category/root pages such as `https://tech.yahoo.com/ai/` and Reuters' artificial-intelligence topic page as source-backed latest items when `news_search` is unavailable. Since #1130 is closed, the remaining highest-value Now-phase refinement is to keep category-only evidence in limited-evidence/unavailable framing while preferring concrete article URLs/snippets for lead bullets whenever available.
