...
1. **Keep AI-news fallback bullets clean when only directory evidence is available** ([#1144](https://github.com/micro/mu/issues/1144)). The 2026-07-05 live guest review after PR #1141 shows the core web ask → answer loop is still fast and largely useful: weather answers in about 3.7s with the correct Sunday, 5 July 2026 anchor; markets answers in under 1s with readable movers and prices; and AI-news fallback responds in about 1s with provider-unavailable disclosure and full source URLs. The remaining top Now-phase friction is that `Find today's AI news` still mixes directory/topic evidence into lead bullets (`news.mit.edu/topic/artificial-intelligence2`, `aimagazine.com`, `tech.yahoo.com/ai/`) and one snippet-promoted story rendered duplicated nested `<strong>` markup. Since #1137 is closed by PR #1141, the next refinement should keep category-only evidence in limited-evidence framing, prefer concrete article URLs/snippets whenever available, and keep rendered bullets clean.
