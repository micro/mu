...
1. **Keep AI-news fallback leads article-specific on live** ([#1119](https://github.com/micro/mu/issues/1119)). The 2026-07-04 live guest review after PR #1117 shows the core web ask → answer loop remains fast and mostly polished: weather answers in about 3s with the correct Saturday, 4 July 2026 anchor, markets starts answering in about 1s with readable movers, and AI-news fallback responds quickly with provider-unavailable disclosure plus source links. Since #1114 is closed, the remaining highest-value Now-phase refinement is the still-live fallback presentation gap: `Find today's AI news` can still name a root/domain result (`The Information`) and generic category-page labels such as `AI News, Updates, Products and Reviews | Yahoo Tech` / `AI (artificial intelligence) | The Guardian` as current items when the useful evidence is only in snippets. The fallback should lead with concrete article/story labels and article URLs, or explicitly disclose limited evidence when only root/topic/category pages are available.
