...
1. **Ground news fallback answers in source snippets** ([#969](https://github.com/micro/mu/issues/969)). Live guest review on 2026-07-01 after PR #967 found the topic-specific fallback is improved: `Find today's AI news` emits progress immediately, discloses `news_search — unavailable`, falls back to web search, streams a dated Markdown answer, and stays on AI rather than broad technology/sports filler. The remaining first-run friction is evidence quality: the answer can over-specify concrete AI headlines from generic source/category pages or weak snippets, making a fast, well-formatted response feel less trustworthy. Rank this first because the Now phase protects correct, source-grounded answers and graceful provider degradation; keep `/agent`, MCP/A2A/REST, and provider env contracts unchanged while requiring fallback synthesis to include only snippet-supported stories or clearly disclose limited evidence.

### Already shipped (do not re-queue)

- ✅ **Unavailable news fallbacks kept topic-specific.** Guest `Find today's AI news` now keeps the web fallback focused on AI instead of broad technology/sports filler, closing #964 / PR #967; remaining source-evidence grounding is tracked in #969.
- ✅ **Live news fallback restored when news_search is unavailable.** Guest `Find today's AI news` now emits early progress, discloses the unavailable news provider, falls back to web results, and streams a readable source-linked answer, closing #959 / PR #962; topic precision later shipped in #964 / PR #967, and remaining source-evidence grounding is tracked in #969.
- ✅ **Guest agent answer latency reduced.** Market/weather/news prompts now emit early progress and PR #957 reduced first-token latency for the market-mover path, closing #951 / PR #957; unavailable-news topic precision later shipped in #964 / PR #967, and remaining source-evidence grounding is tracked in #969.
