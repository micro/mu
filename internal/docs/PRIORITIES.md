...
1. **Require sessions before invoking MCP auth handlers** ([#1079](https://github.com/micro/mu/issues/1079)). This is not a new product surface, but it is the only remaining open codex-labelled Now-phase hardening item after the deployed weather date-anchoring fix shipped. It protects authenticated service operations from running with an empty account binding while preserving the current public MCP contract and keeping the first-run ask → answer loop safe as usage moves between guest, signed-in, and agent-client contexts.
