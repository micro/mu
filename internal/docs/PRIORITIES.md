...
1. **Keep AI-news answers fresh and metadata links clean** ([#1167](https://github.com/micro/mu/issues/1167)). The 2026-07-05 live guest review after PR #1165 shows the restored `news_search` path is now fast and mostly source-clean, so #1162 is done and must not stay at the top of the queue. `Weather in New York today` completed in about 2.8s with the correct Sunday, 5 July 2026 anchor and dated forecast rows, and `What is moving in markets?` completed in about 0.6s with readable prices. The remaining highest-value Now-phase friction is that `Find today's AI news` completed in under 1s but led with May 2026 TechCrunch results for a "today" prompt on 2026-07-05, and its category/date metadata rendered awkwardly with a `Tech` link whose href was the timestamp. Keep the fast provider path, but make freshness disclosure/selection and metadata rendering seamless.
