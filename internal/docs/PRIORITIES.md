...
1. **Keep live fallback answers answer-first after PR 1029** ([#1031](https://github.com/micro/mu/issues/1031)). Live guest review on 2026-07-03 after PR #1029 found the core ask → answer loop is still fast and functional, but the deployed fallback answers still miss the intended result-first bar: `Weather in New York today` completes in about 3.2s with Google Weather data but starts with observation/provider-timestamp bullets and omits the visible current `30°C, Clear` condition; `What is moving in markets?` completes in about 0.5s and is mostly useful, but still needs the answer-first guard preserved without operational preamble; and `Find today's AI news` completes in about 1.4s using web fallback when `news_search` is unavailable but reads like search-result/category bullets rather than concise snippet-grounded synthesis. Preserve the shipped latency, tool progress, source links, freshness metadata, and provider-unavailable transparency while making the first visible sentence directly answer the prompt and moving operational context below it.
