...
1. **Filter broad finance items from AI-news answers** ([#1336](https://github.com/micro/mu/issues/1336)). The 2026-07-09 product review for #1335 found #1331 closed by PR #1334, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page says Mu is “An agent for everyday”, explains news/mail/search/weather/markets/video, exposes suggestion pills, and states “No account needed for your first 3 questions.” Guest `/chat` correctly degrades to an account-required notice with a link back to the public agent path, so signed-in-only chat is not a higher-value guest blocker. Guest `Weather in New York today` is correct, dated to Thursday, 9 July 2026, source-backed, and fast (~1.3s). Guest `What is moving in markets?` remains concise and very fast (~0.9s), with readable prices and the markets card/reference context. PR #1334 improved AI-news topicality: live guest `Find today's AI news` now starts with same-day AI/data-center and AI-infrastructure stories and completes in ~1.0s. The highest-value remaining Now refinement is still AI-news trust, but narrowed: the same answer still promotes broad finance/stock-mover items (for example a generic midday movers headline) as normal AI-news bullets. For a first-run topical ask, weakly related market headlines should be filtered or explicitly demoted as adjacent evidence rather than mixed into the core answer.
