...
1. **Demote broad AI-chip finance background in AI-news answers** ([#1409](https://github.com/micro/mu/issues/1409)). The 2026-07-11 product review for #1408 found #1404 closed by PR #1407, with no remaining open codex-labeled issues or PRs before filing this follow-up. Canon still puts Now-phase seamlessness first: the highest-value work is making the existing guest ask → answer loop correct, fast, well-formatted, and honest before adding surface area. Live first-run remains clear for guests: the landing page says `An agent for everyday`, `/agent` is directly usable, `/chat` explains saved chat requires sign-in while linking to the public agent, and `/weather` explains guest weather access through the agent. Sampled service pages (`/`, `/agent`, `/chat`, `/weather`, `/markets`, `/news`, `/search`, `/video`, `/blog`) returned 200s. Guest loop checks were fast and readable: `Weather in New York today` returned a dated Saturday, 11 July 2026 forecast in about 1.2s, and `What is moving in markets?` returned priced crypto movers in about 1.2s. The remaining top friction is narrower: `Find today's AI news` now correctly leads with a no-current-results freshness caveat for 2026-07-11 and filters the previous generic software/off-topic leftovers, but its background set still starts with broad SK Hynix IPO / stock-mover finance stories before more substantive AI-agent and AI-model governance items. Refinement ranks above additions, so the next increment should keep the freshness caveat while demoting chip-boom finance context unless the story has concrete AI product, model, agent, infrastructure deployment, governance, safety, or user-impact substance.
