...
1. **Lead stale AI-news results with a current-results caveat** ([#1221](https://github.com/micro/mu/issues/1221)). Guest `Find today's AI news` is fast and source-linked, but the live 2026-07-07 review still saw the answer present March-May 2026 stories as today's news before one July item and without an up-front no-current-results/stale-results caveat. This is the highest-value Now-phase refinement because it sits directly in the first-run ask → answer loop: keep the fast `news_search` path and links, but make stale-only or mostly stale current-news answers disclose the freshness gap before listing older context, with regression coverage for the native-stream/synthesis path.
