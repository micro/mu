...
1. **Keep live weather answers from reporting usable forecasts unavailable** ([#1003](https://github.com/micro/mu/issues/1003)). Live guest review on 2026-07-02, after PR #1001 closed #997, still found `Weather in New York today` returning `Unavailable: weather_forecast` while the References card contained Google Weather current conditions, dated forecast rows, and freshness metadata. Rank this first because weather is a core everyday service in the Now phase and this is still the highest-friction ask → answer failure: Mu has the right data, but gives the user the wrong answer. Keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged, preserve fast true-provider-failure degradation, and render a concise location/date-specific answer with source/freshness when the tool payload is usable.
2. **Synthesize news web fallbacks into concise answers** ([#998](https://github.com/micro/mu/issues/998)). Guest `Find today's AI news` still completes quickly and remains topic-focused when `news_search` is unavailable, but the final answer exposes internal scaffolding (`Grounding rule`, raw result-list framing, and generic category pages) instead of a polished, limited-evidence user answer. Rank this after weather because it is also in the core loop and already has working fallback data; the remaining gap is refinement of formatting/truthfulness rather than new surface area. Preserve source links, topic specificity, and the human-readable unavailable-news disclosure while hiding internal grounding instructions.
