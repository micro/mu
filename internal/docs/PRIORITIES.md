...
1. **Polish AI-news answer bullets for readability** ([#1341](https://github.com/micro/mu/issues/1341)). The 2026-07-09 product review for #1340 found #1336 closed by PR #1339, no open codex PRs, and no other open codex-labeled issues before this run. Guest first-run framing remains clear: the landing page states Mu is “An agent for everyday”, explains the everyday-service scope, shows suggestion pills, and tells guests “No account needed for your first 3 questions.” Guest `/chat` remains an acceptable signed-in-only degradation with account-required copy and a public-agent path. Guest `Weather in New York today` is correct, dated to Thursday, 9 July 2026, source-backed, and fast (~1.8s). Guest `What is moving in markets?` is readable, concise, and very fast (~1.0s). PR #1339 fixed the prior broad-finance leakage: live guest `Find today's AI news` now returns same-day, AI-related items in ~0.8s. The highest-value remaining Now refinement is presentation trust in that same first-run news path: bullets still expose feed/category/source metadata and full URLs inline, with truncated snippet fragments such as “using your pu…” / “do not h…”. For a canonical first prompt, the answer should read like a concise source-linked brief rather than raw feed rows.
