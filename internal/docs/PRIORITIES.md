...
1. **Keep live AI-news answers fresh after stale replay ordering guard** ([#1304](https://github.com/micro/mu/issues/1304)). The 2026-07-08 product review for #1303 found #1299 closed by PR #1302, no open codex PRs, and no other open codex issues before this run. Guest `Weather in New York today` remains correct and source-grounded (~3.7s), and `What is moving in markets?` remains concise and fast (~0.8s). However, the live guest `Find today's AI news` path still completes quickly (~1.0s through `news_search`) while leading with May/March 2026 stories ahead of a 2 July 2026 item and without an up-front mostly-stale/no-current-results caveat. This remains the highest-value Now refinement because it is the clearest live truthfulness failure in the first-run ask → answer loop after the latest stale-replay ordering guard: the next increment should cover the deployed-style streaming/final synthesis path so fresh dated items are not buried behind stale replayed stories, or a stale-results caveat appears before older evidence.
