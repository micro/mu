...
1. **Restore live weather-backed agent answers** ([#992](https://github.com/micro/mu/issues/992)). Live guest review on 2026-07-02 found the core market and AI-news prompts now emit early progress and complete in under 1s after #980 / PR #983 and #987 / PR #990, but `Weather in New York today` still completes in about 3s with only the graceful unavailable state (`Unavailable: weather_forecast`). Rank this first because the Now phase requires every existing agent-callable service to work end to end and degrade gracefully when a provider is down; the graceful fallback is present, but live weather is still a core everyday service and should return a location-specific answer with source/freshness context when the provider is healthy. Keep `/agent`, MCP/A2A/REST, webhook, and env contracts unchanged while restoring the live weather path and preserving the fast explicit unavailable-provider fallback.

### Already shipped (do not re-queue)

- ✅ **AI-news fallback answer synthesis latency reduced.** Guest `Find today's AI news` now emits early progress, discloses the unavailable `news_search` provider, falls back to web results, and completes with a readable source-linked answer in about 1s, closing #987 / PR #990; live weather-backed answer restoration is now tracked in #992.
