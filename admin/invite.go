...
func InviteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if app.EmailSender != nil {
			plain := fmt.Sprintf("You've been invited to join Micro.\n\nSign up here: %s\n\nThis link is single-use.", link)
			html := fmt.Sprintf(`<p>You've been invited to join Micro.</p><p><a href="%s">Sign up here</a></p><p>This link is single-use.</p>`, link)
			if err := app.EmailSender(email, "You're invited to Micro", plain, html, ""); err != nil {
