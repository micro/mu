	...
	if r.Method == "POST" {
		...
		if app.SendsJSON(r) {
			...
			if IsExternalEmail(to) {
				if !acc.Admin {
					canProceed, _, cost, err := billing.CheckQuota(acc.ID, billing.OpExternalEmail)
				...
				if !acc.Admin {
					billing.ConsumeQuota(acc.ID, billing.OpExternalEmail) //nolint:errcheck
				}
				SendMessage(acc.Name, acc.ID, to, to, subject, body, replyTo, messageID) //nolint:errcheck
			} else {
				// Internal mail costs credits
				if !acc.Admin {
					canProceed, _, cost, _ := billing.CheckQuota(acc.ID, billing.OpMailSend)
					if !canProceed {
				...
				if !acc.Admin {
					billing.ConsumeQuota(acc.ID, billing.OpMailSend) //nolint:errcheck
				}
			}
		...
		if IsExternalEmail(to) {
			...
			if !acc.Admin {
				canProceed, useFree, cost, err := billing.CheckQuota(acc.ID, billing.OpExternalEmail)
				if err != nil || !canProceed {
			...
			if !acc.Admin {
				if err := billing.ConsumeQuota(acc.ID, billing.OpExternalEmail); err != nil {
