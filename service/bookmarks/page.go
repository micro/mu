...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if app.WantsJSON(r) {
			app.RespondError(w, 401, "Sign in to save this item")
			return
		}
	...
	if ref := r.URL.Query().Get("item"); ref != "" {
		...
		b.WriteString(`<nav class="form-actions" aria-label="Back"><a href="/bookmarks">Back to bookmarks</a></nav><h2>` + html.EscapeString(item.Title) + `</h2><p>` + html.EscapeString(item.Excerpt) + `</p>`)
