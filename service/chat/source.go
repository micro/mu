...
func (Server) Source(ctx context.Context, req *service.SourceRequest, rsp *service.SourceResponse) error {
	...
	for _, m := range said[owner] {
		if m.ID == req.ID {
			rsp.Item = &service.SourceMessage{ID: m.ID, Ref: m.ID, Conversation: m.Conv, Text: m.Text, From: m.From, To: m.To, At: m.At}
			if sender := accountFor(m.From); sender != nil && sender.ID == owner {
